    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-primary/95 backdrop-blur-md border-b border-gold/10">
      <div className="container flex items-center justify-between h-20">
        ...
        <Link to="/" className="flex items-center gap-2">
          <span className="font-display text-2xl font-bold text-secondary tracking-wide">
            DKO
          </span>
          <span className="text-primary-foreground/60 text-xs font-body uppercase tracking-[3px] hidden sm:block">
            Proyectos
          </span>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-8">
          ...
          <DropdownNav
            label="Ventas"
            items={ventas}
            open={openDropdown === "ventas"}
            onToggle={() => setOpenDropdown(openDropdown === "ventas" ? null : "ventas")}
            onClose={() => setOpenDropdown(null)}
          />
        ...
        {/* CTA */}
        <a
          href="https://wa.me/573138765825"
          target="_blank"
          rel="noopener noreferrer"
          className="hidden lg:inline-flex btn-gold text-xs py-2 px-6"
        >
          Cotiza Gratis
        </a>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden bg-primary border-t border-gold/10 overflow-hidden"
          >
...
const NavItem = ({ href, label }: { href: string; label: string }) => (
  <Link
    to={href}
    className="text-primary-foreground/70 hover:text-secondary transition-colors text-sm font-body font-medium uppercase tracking-wider"
  >
    {label}
  </Link>
);
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 8 }}
          className="absolute top-full left-0 mt-2 w-64 bg-card border border-border rounded shadow-lg overflow-hidden"
        >
          {items.map((item) => (
            <Link
              key={item.href}
              to={item.href}
              onClick={onClose}
              className="block px-5 py-3 text-sm font-body text-foreground hover:bg-muted hover:text-accent transition-colors border-b border-border last:border-0"
            >
              {item.label}
            </Link>
          ))}
...
const MobileLink = ({ href, label, onClick }: { href: string; label: string; onClick: () => void }) => (
  <Link
    to={href}
    onClick={onClick}
    className="text-primary-foreground/80 hover:text-secondary text-base font-body font-medium uppercase tracking-wider py-1"
  >
    {label}
  </Link>
);
  ...
  return (
    <div>
      <button
        ...
        className="flex items-center gap-2 text-primary-foreground/80 hover:text-secondary text-base font-body font-medium uppercase tracking-wider py-1 w-full"
      >
        {label}
        ...
        <div className="pl-4 mt-2 flex flex-col gap-2 border-l-2 border-gold/30">
          {items.map((item) => (
            <Link
              ...
              to={item.href}
              onClick={onItemClick}
              className="text-primary-foreground/60 hover:text-secondary text-sm font-body py-1"
            >
              {item.label}
            </Link>
          ))}
