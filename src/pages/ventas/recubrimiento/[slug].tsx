...
const RecubrimientoProduct = () => {
  ...
  const subProducts = slug ? recubrimientoItems[slug] : undefined;
      ...
      {data.heroImage && (
        <section className="relative h-[35vh] min-h-[260px]">
          ...
          <div className="absolute top-6 left-6">
            <Link to="/productos/recubrimiento" className="inline-flex items-center gap-2 px-4 py-2 bg-card/90 backdrop-blur-sm rounded-sm text-sm font-body text-foreground hover:bg-card transition-colors">
              ← Recubrimiento
            </Link>
          </div>
        </section>
              ...
              {subSlugs.map((subSlug, i) => {
                ...
                return (
                  <motion.div key={subSlug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                    <Link
                      to={`/productos/recubrimiento/${slug}/${subSlug}`}
