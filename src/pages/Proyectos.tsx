    ...
    <PageLayout>
      ...
      <section className="py-20 bg-background">
        <div className="container">
            ...
            {projectFilters.map((f) => (
              <button
                ...
                className={`px-5 py-2 text-xs font-body font-semibold uppercase tracking-wider border transition-all ${
                  active === f ? "bg-secondary text-card border-secondary" : "bg-transparent text-muted-foreground border-border hover:border-secondary/50"
                }`}
              >
                {f}
              </button>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((p, i) => (
              <motion.div key={p.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="group relative overflow-hidden rounded-sm cursor-pointer">
                <img src={p.image.src} alt={p.image.alt} className="w-full h-[300px] object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <section className="py-20 bg-muted/30">
        <div className="container max-w-2xl text-center">
          ...
          <ContactForm context="Información sobre proyectos" variant="general" />
        </div>
      </section>
    </PageLayout>
  );
